...
@File: b7_SSDLoss.py
@Time: 2020-03-19 15:21
@Last_update: 2020-03-19 15:21
@Desc: None
@==============================================@
@      _____             _   _     _   _       @
...
class SSDLoss:
    ...
    def smooth_L1_loss(self, y_true, y_pred):
        ...
        '''
        # smooth L1损失就是
        # 如果L1 loss大于1，则返回L1-0.5
        # 如果L1 loss小于1，则返回0.5*L1^2
    ...
    def compute_loss(self, y_true, y_pred):
        ...
        # 每个batch的反例最小值设定 这里给的值是0
        self.n_neg_min = tf.constant(self.n_neg_min)
        # 在计算总体loss时，坐标的回归loss占的权重 这里给的值是1.0
        ...
        # 得到y_true的第0维，也就是得到背景类的mask，(batch, 8732)
        negatives = y_true[:,:,0] # Tensor of shape (batch_size, n_boxes)
        # 得到所有除背景以外的分类结果，如果有分类结果的则为1否则则为0
        # 相当于正例的mask(batch, 8732)
        ...
        # classification_loss(batch,8732),positives(batch,8732)
        # 相乘得到所有正例的交叉熵分类损失，(batch,)
        pos_class_loss = tf.reduce_sum(classification_loss * positives, axis=-1) # Tensor of shape (batch_size,)

        # Compute the classification loss for the negative default boxes (if there are any).
        # 计算范例的分类损失
        # First, compute the classification loss for all negative boxes.
        # classification_loss(batch,8732),negatives(batch,8732)
        # 计算范例的分类损失，(batch,8732)
        neg_class_loss_all = classification_loss * negatives # Tensor of shape (batch_size, n_boxes)
        # 得到范例的数量，也就是非零的数量
        # 这里需要非零的数量是因为在计算tf.nn.top_k进行范例选择的时候如果所有范例都是零则可能会选到正例
        # 所以这里通过非零的数量来给tf.nn.top_k来摆脱这种情况
        ...
        # 这里得到要选择多少个范例
        # self.neg_pos_ratio * tf.to_int32(n_positive)是计算按照范例比率应该选择多少个范例
        # 然后和self.n_neg_min进行比较，如果小于self.n_neg_min则按照self.n_neg_min来进行选择
        # 最后和n_neg_losses进行比较，如果比n_neg_losses大，则按照n_neg_losses来进行，去除可能选到正例的情况
        ...
        # 我们只计算正例的回归损失，因为只有正例有坐标位置，(batch,)
        loc_loss = tf.reduce_sum(localization_loss * positives, axis=-1) # Tensor of shape (batch_size,)

        # 4: Compute the total loss.
        # 计算总体损失，这里使用tf.maximum是为了避免除零错误
        # 另外除n_positive是为了得到平均值，避免正例多少引起的损失抖动
        ...
        # 这里再乘batch是因为n_positive是全部batch的汇总
...
if __name__ == '__main__':
    # y_true, y_pred, (batch, 8732, 21+4+8)
    # y_true为类别+预测框+先验框+variance
    # y_label为类别+label坐标位置+先验框+variance
